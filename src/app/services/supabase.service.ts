...
export class SupabaseService {
  ...
  addTodo(task: string, coupon: string, link: string, desc: string) {
    const userId = this.getSession()?.user?.id as string
    return this.supabaseClient.from('todos').insert({ task, coupon, link, desc }).single()
  }
  ...
  deleteTodo(coupon: string) {
    return this.supabaseClient.from('todos').delete().eq('coupon', coupon)
  }
}
